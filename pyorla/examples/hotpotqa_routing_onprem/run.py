"""HotpotQA evaluation with LangGraph + Orla on self-hosted vLLM.

Identical multi-stage decomposition pipeline to ``hotpotqa_routing`` but
targeting on-premise GPUs running vLLM instead of Bedrock Mantle.  Each
question passes through up to four LLM stages (triage, decompose, answer
sub-questions, synthesize), producing 5-7 inference calls.  With self-hosted
models the per-call TTFT and TPS differences are far larger than on managed
APIs, so the aggregate latency gains from routing are more pronounced.
...
Model tiers (two-tier, each on its own GPU):

- **Cheap**: Qwen3 4B Instruct on GPU 1.  Expected ~30-60ms TTFT, 300-500 t/s.
- **Strong**: Qwen3 32B on GPU 0.  Expected ~200-400ms TTFT, 40-70 t/s.

Override ``HOTPOTQA_CHEAP_MODEL``, ``HOTPOTQA_STRONG_MODEL``,
``HOTPOTQA_CHEAP_ENDPOINT``, and ``HOTPOTQA_STRONG_ENDPOINT`` via env vars.

Prerequisites:

- Two vLLM instances serving the models on separate GPUs.  Start them with
...
Run from the ``pyorla`` directory::
    ...
    uv run python examples/hotpotqa_routing_onprem/run.py --mode routed --limit 20
    uv run python examples/hotpotqa_routing_onprem/run.py --mode baseline --limit 20
    uv run python examples/hotpotqa_routing_onprem/run.py --mode all-cheap --limit 20
...
def _register_backends(client: OrlaClient) -> LLMBackend:
    """Register cheap / strong backends for on-prem vLLM.
    ...
    Two-tier setup: each model runs on its own GPU.  Cost models use
    notional per-token prices that reflect relative GPU-time cost so
    Orla's cost-aware routing picks the correct tier.
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "on self-hosted vLLM (multi-stage decomposition pipeline)."
        ),
    )
    parser.add_argument(
        "--mode",
